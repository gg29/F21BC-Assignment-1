...
Title: Part II - GA Version 4 COCO
Description: Includes HillClimbing, Mutation, Multiple Dimensions,
Crossover and Roulette Selection
%}

function xbest = GA_OPTIMIZER(FUN, DIM, ftarget, maxfunevals)

  popsize = 1000; %Population Size
  maxfunevals = 500; %Number of generations
  ...
  for currentGeneration = 1:ceil(maxfunevals/popsize)
    ...
    individuals = rouletteSelection(individuals); %Do Roulette Selection  
...
function [newMatrix] = rouletteSelection(individuals)
    
    [rows,columns] = size(individuals);
    individualScores = zeros(1, rows);

    %Evaluate each pair and save it to vector
    for r= 1:rows
        individualScores(1,r) = sphereFunction(individuals(r,:));
    end
    
    overallFitness = sum(individualScores); %Calculate overall fitness
    weights = zeros(rows,1); %Initialize a vector with weights
    
    %Calculate weights for each pair
    for r=1:rows
        weights(r,1) = individualScores(1,r)/overallFitness;
    end
   
    newMatrix = zeros(rows,columns); %Initialize a new vector
    
    %Insert new values in newMatrix with probability = weight
    try
        for i = 1: rows
            ind = randsample(1:rows, 1, true, weights);
            newMatrix(i,:) = individuals(ind,:);
        end
    end
